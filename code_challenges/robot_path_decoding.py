"""
Google kickstart coding challenge Round B Q3
**Problem**
...
num_tcases = sys.stdin.readline()
num_tcases = int(num_tcases)

for tc in range(num_tcases):
    mul_stack = []
    ...
    for move in moves:
        if move == ')':
            scalar /= mul_stack.pop()
        elif move in ['2', '3', '4', '5', '6', '7', '8', '9']:
            mul_stack.append(int(move))
            scalar *= mul_stack[-1]
        elif move in ['S', 'W', 'E', 'N']:
            if move == 'S':
                y = y + scalar * 1
            elif move == 'N':
                y = y - scalar * 1
